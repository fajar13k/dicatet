...
function RegisterInput({ register }) {
  ...
  return (
    <form onSubmit={onSubmitHandler} className="appearance-none w-1/2">
      <div className="mb-4">
        <label htmlFor="name" className="block mb-2 text-md font-medium">Name</label>
        <input type="text" id="name" className="bg-slate-700 font-semibold text-lg rounded-md block w-full p-2.5 focus:outline focus:outline-pink-600" placeholder="(Your name)" value={name} onChange={handleNameChange} />
      </div>
      <div className="mb-4">
      ...
      <button type="submit" className="bg-gradient-to-r bg-pink-600 hover:bg-pink-700 px-4 py-2 font-bold rounded-md focus:outline focus:outline-pink-400">Register</button>
    </form>
  )
}
