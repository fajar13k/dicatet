...
function RegisterInput({ register }) {
  ...
  return (
    <form onSubmit={onSubmitHandler} className="appearance-none w-1/2">
      <div className="mb-4">
        <label htmlFor="name" className="block mb-2 text-md font-medium text-slate-700 dark:text-neutral-50">Name</label>
        <input type="text" id="name" className="bg-neutral-200 dark:bg-slate-700 text-slate-700 dark:text-neutral-50 font-semibold text-lg rounded-md block w-full p-2.5 focus:outline focus:outline-pink-600" placeholder="(Your name)" value={name} onChange={handleNameChange} />
      </div>
      <div className="mb-4">
      ...
      <button type="submit" className="text-neutral-50 bg-pink-600 hover:opacity-80 px-4 py-2 font-bold rounded-md focus:outline focus:outline-pink-400">Register</button>
    </form>
  )
}
